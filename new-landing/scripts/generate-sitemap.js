...
-->
${allRoutes.map(route => {
    const loc = route.startsWith('http://') || route.startsWith('https://')
        ? route
        : `${BASE_URL}${route}${route.endsWith('/') || route.length === 1 ? '' : '/'}`;
    return `  <url>
    <loc>${loc}</loc>
    ...
    // 2. Generate Images Sitemap (only image URLs)
    const imagesDir = path.join(__dirname, '../public/images');
    const imageSet = new Set();
    const validExtensions = ['.webp', '.png', '.jpg', '.jpeg', '.svg', '.gif'];
    ...
    if (fs.existsSync(imagesDir)) {
        ...
        imageFiles.forEach(file => {
            ...
            if (validExtensions.includes(ext)) {
                imageSet.add(`${BASE_URL}/images/${encodeURI(file)}`);
            }
        });
    }

    // Also include any images from authors if not already present
    Object.values(authorImages).forEach(img => {
        if (img) {
            const fullUrl = img.startsWith('http://') || img.startsWith('https://')
                ? img
                : `${BASE_URL}${img.startsWith('/') ? '' : '/'}${encodeURI(img.replace(/^\//, ''))}`;
            imageSet.add(fullUrl);
        }
    });

    const allImages = Array.from(imageSet).sort();
    console.log(`🖼️  Found ${allImages.length} images for image sitemap.`);
