...
/**
 * Loads author data for image mapping
 */
function loadAuthors() {
    ...
    // Extract images using regex since it's a TS file
    const authorRegex = /"([^"]+)":\s*{[\s\S]*?"image":\s*"([^"]+)"/g;
    const authorImages = {};
    let match;
    
    while ((match = authorRegex.exec(content)) !== null) {
        authorImages[match[1]] = match[2];
    }
...
function getBlogPosts() {
    ...
    const authorImages = loadAuthors();
